...
| xml 설정 파일 작성(Spring 2.5) | auto-scan 기능이 추가되어 빈을 등록하기 위해 `@Component`어노테이션을 클래스 위에 붙여줌, xml 설장 파일에 빈을 직접 등록하는 것보다 쉬움 |
| Java Config                    | java 설정파일을 만들어 빈을 등록하고 `ApplicationContext`의 `AnnotationConfigApplicationContext`를 통해 빈 팩토리를 받아옴 |
